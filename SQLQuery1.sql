...
CREATE DATABASE VehiculosTestDb;
GO
...
CREATE UNIQUE INDEX UX_Vehiculo_Matricula ON Vehiculo(Matricula);
GO
-- Conductor
...
CREATE UNIQUE INDEX UX_Conductor_NumeroLicencia ON Conductor(NumeroLicencia);
GO
...
ON Asignacion(VehiculoId, ConductorId, FechaInicio, FechaFin);
GO

-- Índices adicionales para rendimiento
CREATE INDEX IX_Asignacion_FechaInicio ON Asignacion(FechaInicio);
CREATE INDEX IX_Asignacion_FechaFin ON Asignacion(FechaFin);
...
AS
BEGIN
  INSERT INTO Vehiculo (Matricula, Marca, Modelo, Anio, Tipo, Estado)
...
AS
BEGIN
  INSERT INTO Conductor (NombreCompleto, NumeroLicencia, Contacto)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM Vehiculo WHERE Id=@VehiculoId AND Estado='Asignado')
  BEGIN
  ...
  INSERT INTO Asignacion (VehiculoId, ConductorId, FechaInicio, FechaFin)
  VALUES (@VehiculoId, @ConductorId, @FechaInicio, @FechaFin);

  UPDATE Vehiculo SET Estado='Asignado' WHERE Id=@VehiculoId;
END
GO
...
BEGIN
  ...
  SELECT @VehiculoId = VehiculoId FROM Asignacion WHERE Id=@Id;

  DELETE FROM Asignacion WHERE Id=@Id;

  IF @VehiculoId IS NOT NULL
    UPDATE Vehiculo SET Estado='Disponible' WHERE Id=@VehiculoId;
END
GO
